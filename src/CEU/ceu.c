...
uint32_t g_image_width = 0;
uint32_t g_image_height = 0;
...
//    else if (CEU_EVENT_VD == p_args->event )
//    {
//        g_capture_ready = true;
//    }
}
...
//    APP_ERR_RETURN(err, " ** R_CEU_Open API FAILED ** \r\n");
    ...
    /* Clean image buffer */
    memset(p_buffer, 0, width * height * YUV422_BYTE_PER_PIXEL);
...
void rgb565_to_rgb888(const void* inbuf, void* outbuf, uint16_t width, uint16_t height)
{
    const uint16_t* src = (const uint16_t*)inbuf;
    uint32_t* dst = (uint32_t*)outbuf;
    for (uint32_t i = 0; i < height; i++) {
        for (uint32_t j = 0; j < width; j++){
            dst[width*i+j]=(uint32_t)((src[width*i+j] & 0xF800)<<8) | ((src[width*i+j] & 0x7E0)<<5) | ((src[width*i+j] & 0x1F)<<3);
            dst[width*i+j]|=0xFF070307;
        }
    }
}
