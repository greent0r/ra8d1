...
#define vTaskDelay(x)  R_BSP_SoftwareDelay(x, BSP_DELAY_UNITS_MILLISECONDS)


/* OV5640????????????? */
typedef enum
{
    OV5640_REG_BANK_DSP = 0x00, /* DSP????????? */
    OV5640_REG_BANK_SENSOR,     /* Sensor????????? */
} ov5640_reg_bank_t;
...
uint8_t g_image_vga_sram  [ VGA_WIDTH * VGA_HEIGHT * RGB565_BYTE_PER_PIXEL ] BSP_ALIGN_VARIABLE(64) BSP_PLACE_IN_SECTION(".sdram");
uint8_t g_image_vga_sram2 [ VGA_WIDTH * VGA_HEIGHT * RGB565_BYTE_PER_PIXEL ] BSP_ALIGN_VARIABLE(64) BSP_PLACE_IN_SECTION(".sdram");
uint8_t rgb888_data [ VGA_WIDTH * VGA_HEIGHT * 3 ] BSP_ALIGN_VARIABLE(64) BSP_PLACE_IN_SECTION(".sdram");
uint8_t jpg_data [ VGA_WIDTH * VGA_HEIGHT] BSP_ALIGN_VARIABLE(64) BSP_PLACE_IN_SECTION(".sdram");
int jpg_len=0;
...
/**
 * @brief       ??????OV5640模???
 * @param       cmos_h_pixel    1024
 * @param       cmos_v_pixel    600
 * @param       total_h_pixel   2200
 * @param       total_v_pixel   1000
 * @retval      OV5640_EOK   : OV5640模?????????????
 *              OV5640_ERROR : ???????????OV5640模?????????失败
 *              OV5640_ENOMEM: ??????足??OV5640模?????????失败
 */
uint8_t ov5640_init(uint16_t cmos_h_pixel, uint16_t cmos_v_pixel, uint16_t total_h_pixel, uint16_t total_v_pixel)
{
    uint16_t chip_id;

    ov5640_hw_init();               /* OV5640模?????件??????? */
    ov5640_exit_power_down();       /* OV5640模????????????模?? */
    ov5640_hw_reset();              /* OV5640模?????件???? */


    ov5640_sw_reset();              /* OV5640模?????件???? */
    vTaskDelay(200);
    chip_id = ov5640_get_chip_id(); /* ?????????ID */
...
//    //设置?????????????
//    ov5640_write_reg(0x3808, cmos_h_pixel >> 8     );  //DVP ????水平?????????????4??//8
//    ov5640_write_reg(0x3809, cmos_h_pixel & 0x00FF );  //DVP ????水平????????????8??//0FF
//    ov5640_write_reg(0x380a, cmos_v_pixel >> 8     );  //DVP ??????????????????????3??//8
//    ov5640_write_reg(0x380b, cmos_v_pixel & 0x00FF );  //DVP ?????????????????????8??//0FF
//    ov5640_write_reg(0x380c, total_h_pixel >> 8    );  //水平???????大????5??
//    ov5640_write_reg(0x380d, total_h_pixel & 0x00FF);  //水平???????大????8??
//    ov5640_write_reg(0x380e, total_v_pixel >> 8    );  //????????????大????5??
//    ov5640_write_reg(0x380f, total_v_pixel & 0x00FF);  //????????????大????8??
//
//    //翻转
//    ov5640_write_reg(0x3820,0x46);  //????翻转40/46,46??翻转       Flip
//    ov5640_write_reg(0x3821,0x06);  //????翻??01/07,01??认?????  Mirror
//
...
//    ov5640_write_reg(0x3a02,0x17); //60Hz max exposure
//    ov5640_write_reg(0x3a03,0x10); //60Hz max exposure
//    ov5640_write_reg(0x3a14,0x17); //50Hz max exposure
//    ov5640_write_reg(0x3a15,0x10); //50Hz max exposure
//    ov5640_write_reg(0x4004,0x02); //BLC(????) 2 lines
...
//    ov5640_write_reg(0x5001,0xa3); //ISP ?????
//    ov5640_write_reg(0x3b07,0x0a); //帧?????模??
//    ov5640_write_reg(0x503d,0x00); //0x00:正常模??




    fsp_err_t err = ceu_init(g_image_vga_sram, VGA_WIDTH, VGA_HEIGHT);
    if(err != FSP_SUCCESS )
    {
        printf( "** ceu_operation API failed ** \r\n");
        return OV5640_ERROR;
    }
...
void restore_rgb565_from_swapped()
{
    uint8_t temp;
    for (size_t i = 0; i < 320*240*2; i += 2) {
        // 每个像素是 2 字节，如果 Swap 16-bit，则需要交换这两个字节
        g_image_vga_sram2[i]=g_image_vga_sram[i+1];
        g_image_vga_sram2[i+1]=g_image_vga_sram[i];
    }
}
 ...
 * @param       ????????????????????
 * @retval      OV5640_EOK     : OV5640模???????????????????????
 *              OV5640_ERROR   : OV5640模?????????????
 */
uint8_t ov5640_capture()
{
    ...
    restore_rgb565_from_swapped();
    //rgb565_to_rgb888(&g_image_vga_sram[0], g_image_rgb888_buffer, VGA_WIDTH, VGA_HEIGHT);
    //yuv422_to_rgb888(&g_image_vga_sram[0], p_buffer, VGA_WIDTH, VGA_HEIGHT);
    return OV5640_EOK;
}
 ...
 * @brief       OV5640模?????????????????
 * @param       ??
 * @retval      OV5640_EOK     : OV5640模???????????????次?????
 *              OV5640_ETIMEOUT: OV5640模???????????????次????
 */
 ...
 * @retval      OV5640_EOK   : 设置OV5640模?????彩饱?????????
 *              OV5640_EINVAL: ??????????????
 */
 ...
 * @retval      OV5640_EOK   : 设置OV5640模???????/翻转????
 *              OV5640_EINVAL: ??????????????
 */
...
/**
 * @brief       设置OV5640模???ISP?????????尺??
 * @note        OV5640模???ISP??????????????大尺寸为0x0A3F*0x079F
 * @param       x     : ISP?????????起??X????
 *              y     : ISP?????????起??Y????
 *              width : ISP????????????度
 *              height: ISP??????????????
 * @retval      OV5640_EOK   : 设置OV5640模???ISP?????????尺寸?????
 *              OV5640_EINVAL: ??????????????
 */
...
/**
 * @brief       设置OV5640模?????缩?????????????
 * @note        OV5640模?????缩????????尺寸??须????ISP?????????尺??
 * @param       x_offset: ??缩????????X????
 *              y_offset: ??缩????????Y????
 * @retval      OV5640_EOK   : 设置OV5640模?????缩???????????移?????
 */
