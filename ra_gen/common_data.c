...
/** Display framebuffer */
        ...
        uint8_t g_framebuffer[2][DISPLAY_BUFFER_STRIDE_BYTES_INPUT0 * DISPLAY_VSIZE_INPUT0] BSP_ALIGN_VARIABLE(64) BSP_PLACE_IN_SECTION(".sdram");
        #else
...
{ .tcon_hsync = GLCDC_TCON_PIN_0,
  ...
  .clock_div_ratio = GLCDC_PANEL_CLK_DIVISOR_12,
...
const display_cfg_t g_display_cfg =
        ...
        .input[0] =
        {
          ...
          .output =
                  { .htiming =
                  { .total_cyc = 1344, .display_cyc = 1024, .back_porch = 140, .sync_width = 20, .sync_polarity =
                            DISPLAY_SIGNAL_POLARITY_LOACTIVE },
                    .vtiming =
                    { .total_cyc = 635, .display_cyc = 600, .back_porch = 20, .sync_width = 3, .sync_polarity =
                              DISPLAY_SIGNAL_POLARITY_LOACTIVE },
          ...
          .p_callback = glcdc_vsync_callback,
          .p_context = NULL,
